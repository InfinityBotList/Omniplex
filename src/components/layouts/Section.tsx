...
const backgroundStyles: Record<BackgroundVariant, string> = {
  none: '',
  brand: 'bg-gradient-to-b from-transparent via-brand-500/[0.02] to-transparent',
  accent: 'bg-gradient-to-b from-transparent via-brand-500/[0.02] to-transparent',
...
export function SectionHeader({
  ...
  return (
    <div className={`flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-12 ${className}`}>
      <div>
        {badge && (
          <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full glass text-xs font-medium ${badgeTextClass} mb-4`}>
            {badge.icon || (
              <span className={`w-2 h-2 rounded-full ${badgeDotClass} animate-pulse`} />
            )}
            {badge.text}
          </div>
        )}
